...
def person_name(gender_of_id, last_name_file, male_name_file, female_name_file):
    ...
    """
    ls=[]
    f=open(last_name_file,"r",encoding='utf-8')
    for num,line in enumerate(f):
        if num<51:
            for i in [0,1,2,3,5,6,7,8]:
                ls.append(line[i])
        else:
            for i in [0,2,5,7] if num<70 else [0,2]:
                ls.append(line[i:i+2])
    name=random.choice(ls)
    f=open(male_name_file if gender_of_id=='男' else female_name_file,"r",encoding='utf-8')
    ls=f.readline().split()
    f.close()
    return name+random.choice(ls)
...
def area_code(area_file):
    ...
    """
    f=open(area_file,"r",encoding='utf-8')
    d={}
    for l in f:
        l=l.replace('\n','').split(',')
        d[l[0]]=l[1]
...
def id_of_17(area_of_code, birth_date, birth_order):
    ...
    """
    area_id = random.choice([x for x in list(area_of_code.keys()) if x[-2:] != '00'])
...
def id17_to_18(id_number):
    ...
    sum=0
    for i in range(17):
        sum+=w[i]*eval(id_number[i])
    return id_number+t[sum%11]
...
def village_of_live(village_file, area_of_code):
    ...
    return area_of_registration(area_id,area_of_code)+"{}{}栋{}单元{:0>2d}{:0>2d}室".format(village,building,door,floor,room),area_id
...
def area_of_registration(id_number, area_of_code):
    ...
    其他地区格式例如：湖北省武汉市洪山区。
    """
    ans=area_of_code[id_number[:2]+'0000']
    if ans[-1]!='市':
        ans+=area_of_code[id_number[:4]+'00']
    return ans+area_of_code[id_number[:6]]
...
def area_of_live(area_id, area_of_code):
    ...
    其他地区格式例如：湖北省武汉市洪山区。
    """
    ans=area_of_code[area_id[:2]+'0000']
    if ans[-1]!='市':
        ans+=area_of_code[area_id[:4]+'00']
    return "持证人居住于"+ans+area_of_code[area_id]
...
def check_city_code(city_name, area_of_code):
    ...
    """
    d={}
    for k in area_of_code:
        d[area_of_code[k]]=k
    return d[city_name] if city_name in d else False
